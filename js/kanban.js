  ...
  _renderMobileCard(os, isDono, showBadge) {
    ...
    return `
      <div class="kanban-card" onclick="OS.abrirDetalhes('${os.id}')"
        ...
        ${os.status === 'entregue' && !os.pago && os.forma_pagamento !== 'faturado' ? `<div style="background:var(--danger-bg);color:var(--danger);padding:4px 8px;border-radius:var(--radius);font-size:11px;font-weight:700;margin-bottom:8px;text-align:center;">SEM PAGAMENTO DEFINIDO</div>` : ''}
  ...
  _renderCard(os, isDono) {
    ...
    return `
      <div class="kanban-card" draggable="true"
        ...
        ${os.status === 'entregue' && !os.pago && os.forma_pagamento !== 'faturado' ? `<div style="background:var(--danger-bg);color:var(--danger);padding:3px 6px;border-radius:var(--radius);font-size:10px;font-weight:700;margin-top:6px;text-align:center;">SEM PAGAMENTO</div>` : ''}
