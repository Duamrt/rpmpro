...
const OS = {
  ...
  async carregar() {
    ...
    if (this._filtroStatus === 'pendente') {
      query = query.eq('status', 'entregue').eq('pago', false);
    } else if (this._filtroStatus) {
  ...
  render() {
    ...
    const statusLabel = {
      ...
      entregue: 'Entregue', pendente: 'Pendente (nao pago)', cancelada: 'Cancelada'
    };
  ...
  async mostrarAddPeca(tipo) {
    ...
    if (tipo === 'estoque' && !this._pecasEstoque) {
      const { data } = await db.from('pecas')
        .select('id, nome, codigo, marca, preco_venda, custo, quantidade, compatibilidade')
        .eq('oficina_id', APP.oficinaId)
        .order('nome');
      ...
      container.innerHTML = `
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr auto;gap:8px;align-items:end;">
          ...
          <div class="form-group" style="margin:0;">
            ...
            <input type="number" class="form-control" id="det-avulso-custo" placeholder="0.00" min="0.01" step="0.01">
          </div>
          <div class="form-group" style="margin:0;">
  ...
  _buscarPecaEstoque(val) {
    ...
    sugEl.innerHTML = resultados.map(p => {
      ...
      <div style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px;${compat ? 'background:rgba(63,185,80,0.06);' : ''}"
           ...
           onclick="OS._selecionarPecaEstoque('${p.id}','${(p.nome || '').replace(/'/g, '')}',${p.preco_venda || 0},${p.quantidade || 0},${p.custo || 0})">
  ...
  _selecionarPecaEstoque(id, nome, preco, qtdEstoque, custo) {
    ...
    this._pecaSelecionadaCusto = custo || 0;
  },
  ...
  async addPecaEstoque() {
    ...
    // 1. Insere item na OS (com snapshot do custo)
    const custoUnit = this._pecaSelecionadaCusto || 0;
  ...
  async addPecaAvulso() {
    ...
    if (!custo || custo <= 0) { APP.toast('Preencha o custo da peca — sem isso o lucro fica errado', 'error'); return; }
    if (!valorUnit || valorUnit <= 0) { APP.toast('Preencha o valor de venda', 'error'); return; }
    ...
    // Insere item na OS (com custo digitado)
    const { error } = await db.from('itens_os').insert({
      oficina_id,
      os_id: this._osAtualId,
      tipo: 'peca',
