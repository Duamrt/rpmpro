...
const FINANCEIRO = {
  ...
  async _carregarFechamento() {
    ...
    const [osRes, fiadosRes, canceladasRes, caixaRes, maqRes] = await Promise.all([
      ...
      db.from('maquininhas')
        .select('nome, taxa_debito, taxa_credito')
        .eq('oficina_id', oficina_id)
        .eq('ativo', true)
    ]);
    const maquininhas = maqRes.data || [];
    const maqSemTaxa = maquininhas.filter(m => !m.taxa_debito && !m.taxa_credito);
    const _alertaTaxa = maqSemTaxa.length ? `<div style="background:var(--warning-bg, #332b00);border:1px solid var(--warning, #D97706);border-radius:var(--radius);padding:8px 12px;margin-bottom:12px;font-size:12px;color:var(--warning, #D97706);">⚠ ${maqSemTaxa.map(m => m.nome).join(', ')} sem taxa configurada — lucro pode estar incorreto. <a href="#" onclick="APP.navegar('config');return false;" style="color:inherit;text-decoration:underline;">Ajustar</a></div>` : '';
    ...
    if (osIds.length) {
      const { data } = await db.from('itens_os')
        .select('os_id, descricao, quantidade, valor_unitario, valor_total, custo_unitario, peca_id, pecas(nome, custo)')
        .in('os_id', osIds)
        .eq('tipo', 'peca');
      itensPecaDia = data || [];
    }
    ...
    // Lucro peças: vendeu - custou (prioriza custo_unitario do item, fallback pecas.custo)
    let custoPecas = 0;
    let vendaPecas = 0;
    itensPecaDia.forEach(item => {
      vendaPecas += item.valor_total || 0;
      if (item.custo_unitario > 0) {
        custoPecas += item.custo_unitario * (item.quantidade || 1);
      } else if (item.peca_id && item.pecas?.custo) {
      ...
      <div style="margin-bottom:24px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(${_mob ? 2 : 4}, 1fr);gap:10px;">
          ${['dinheiro','pix','debito','credito'].map(f => {
            ...
            const vlrTaxa = osForma.reduce((s, o) => s + ((o.valor_total || 0) * (o.taxa_cartao || 0) / 100), 0);
            ...
            <div style="background:var(--bg-card);border:1px solid var(--border);padding:14px 16px;border-radius:var(--radius);${bruto === 0 ? 'opacity:0.35;' : ''}">
              ...
              ${temTaxa ? `
                <div style="font-size:12px;color:var(--text-muted);margin-top:6px;">taxa -${APP.formatMoney(vlrTaxa)}</div>
  ...
  async _carregarCaixa() {
    ...
    const [caixaRes, osRes] = await Promise.all([
      db.from('caixa')
        ...
        .order('created_at', { ascending: false }),
      db.from('ordens_servico')
        .select('id, numero, valor_total, forma_pagamento, taxa_cartao, data_entrega, descricao, veiculos(placa), clientes(nome)')
        .eq('oficina_id', oficina_id)
        .eq('pago', true)
    ...
    // Peças: custo vs venda (prioriza custo_unitario do item)
    let pecasVenda = 0, pecasCusto = 0;
    pecasMovimentadas.forEach(p => {
      pecasVenda += p.valor_total || 0;
      if (p.custo_unitario > 0) pecasCusto += p.custo_unitario * (p.quantidade || 1);
      else if (p.peca_id && p.pecas?.custo) pecasCusto += p.pecas.custo * (p.quantidade || 1);
    });
    ...
    el.innerHTML = `
      ...
      <div style="margin-bottom:24px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(${_mob ? 2 : 4}, 1fr);gap:10px;">
          ${['dinheiro','pix','debito','credito'].map(f => {
            const bruto = porForma[f] || 0;
            const osFormaArr = osPagas.filter(o => (o.forma_pagamento || 'outros') === f);
            const vlrTaxa = osFormaArr.reduce((s, o) => s + ((o.valor_total || 0) * (o.taxa_cartao || 0) / 100), 0);
            const temTaxa = (f === 'debito' || f === 'credito') && bruto > 0;
            const liquido = bruto - vlrTaxa;
            return `
            <div style="background:var(--bg-card);border:1px solid var(--border);padding:14px 16px;border-radius:var(--radius);${bruto === 0 ? 'opacity:0.35;' : ''}">
          ...
          <table style="width:100%;border-collapse:collapse;">
            ...
            <tbody>
              ${pecasMovimentadas.slice(0, 5).map(p => {
                const custo = p.custo_unitario > 0 ? p.custo_unitario * (p.quantidade || 1) : (p.peca_id && p.pecas?.custo ? p.pecas.custo * (p.quantidade || 1) : 0);
  ...
  async _carregarLucroPecas() {
    ...
    itensPeca.forEach(item => {
      ...
      const custo = item.custo_unitario > 0 ? item.custo_unitario * (item.quantidade || 1) : (item.peca_id && item.pecas?.custo ? item.pecas.custo * (item.quantidade || 1) : 0);
      totalVenda += venda;
      totalCusto += custo;
