...
const FOLHA = {
  ...
  async carregar() {
    ...
    const [membrosRes, osRes, valesRes] = await Promise.all([
      db.from('profiles')
        ...
        .eq('ativo', true)
        .in('role', ['dono', 'mecanico', 'aux_mecanico', 'gerente', 'atendente', 'aux_admin'])
        .order('nome'),
    ...
    const roleLabel = { dono: 'Pró-labore', mecanico: 'Mecânico', aux_mecanico: 'Aux. Mecânico', gerente: 'Gerente', atendente: 'Atendente', aux_admin: 'Aux. Admin' };
    ...
    container.innerHTML = `
      ...
      <div style="margin-top:20px;display:flex;gap:8px;flex-wrap:wrap;">
        ...
        <button class="btn btn-success" id="btn-lancar-folha" onclick="FOLHA.lancarNoCaixa()">Lancar folha no Caixa</button>
      </div>
      `}
    `;
  },
  ...
  async lancarNoCaixa() {
    ...
    // Verifica se já lançou essa folha
    const refFolha = `folha-${ano}-${String(mes).padStart(2, '0')}`;
    const { data: jaLancou } = await db.from('caixa')
      .select('id')
      .eq('oficina_id', oficina_id)
      .eq('categoria', 'retirada')
      .like('descricao', `%${refFolha}%`)
      .limit(1);

    if (jaLancou && jaLancou.length) {
      APP.toast(`Folha de ${mesNome}/${ano} ja foi lancada no caixa`, 'error');
      return;
    }

    // Busca dados da folha
    ...
    const [membrosRes, osRes, valesRes] = await Promise.all([
      db.from('profiles').select('id, nome, role, salario_base, comissao_percent, vale_refeicao')
        .eq('oficina_id', oficina_id).eq('ativo', true)
        .in('role', ['dono', 'mecanico', 'aux_mecanico', 'gerente', 'atendente', 'aux_admin']),
      db.from('ordens_servico').select('valor_total, mecanico_id')
        .eq('oficina_id', oficina_id).eq('status', 'entregue')
        .gte('data_entrega', inicio).lte('data_entrega', fim),
      db.from('caixa').select('membro_id, valor')
        .eq('oficina_id', oficina_id).eq('tipo', 'saida').eq('categoria', 'vale')
        .gte('created_at', inicio).lte('created_at', fim + 'T23:59:59')
    ]);
    ...
    if (!membros.length) { APP.toast('Nenhum funcionario ativo', 'error'); return; }
    ...
    membros.forEach(m => {
      ...
      const com = fat * (m.comissao_percent || 0) / 100;
      const vale = valePor[m.id] || 0;
      const liquido = sal + vr + com - vale;

      if (liquido <= 0) return;

      totalFolha += liquido;
      const roleLabel = m.role === 'dono' ? 'Pro-labore' : 'Salario';
      let desc = `${roleLabel} ${m.nome} — ${mesNome}/${ano} [${refFolha}]`;
      if (com > 0) desc += ` (comissao R$${com.toFixed(2)})`;
    ...
    if (!lancamentos.length) { APP.toast('Nenhum valor a lancar', 'error'); return; }

    // Confirma
    const fmt = v => v.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });
    if (!confirm(`Lancar folha de ${mesNome}/${ano} no Caixa?\n\n${lancamentos.length} funcionarios — Total: ${fmt(totalFolha)}\n\nIsso vai registrar como despesa no financeiro.`)) return;

    const { error } = await db.from('caixa').insert(lancamentos);
    if (error) { APP.toast('Erro: ' + error.message, 'error'); return; }

    APP.toast(`Folha de ${mesNome}/${ano} lancada no Caixa — ${fmt(totalFolha)}`);
    this.carregar();
  }
};
